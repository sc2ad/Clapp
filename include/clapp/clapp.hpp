...
template <class... Ts> struct OverloadSet : Ts... {
  OverloadSet(Ts &&...ts) : Ts(std::move(ts))... {}
  using Ts::operator()...;
};

template <class... Ts> auto MakeOverloadedSet(Ts &&...ts) {
...
// std::array<const char*, NUMARGS(__VA_ARGS__)> const&,
// std::declval<std::invoke_result_t<decltype([](auto t, auto f, auto&... args)
// {return dump_type(t, f, args...);}), decltype(t), decltype(f),
// DECLTYPEIFY_ADD_REF(__VA_ARGS__)>>()
...
// static_assert(same(STR(UNPACK_IMPL((_1)(_2)(_3))), "TEST"));

template <class T, class F> auto do_on_bindings_impl(T &&t, F &&f) {
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wgnu-statement-expression-from-macro-expansion"
  return MakeOverloadedSet(
      UNPACK_IMPL((_1)), UNPACK_IMPL((_1)(_2)), UNPACK_IMPL((_1)(_2)(_3)),
      UNPACK_IMPL((_1)(_2)(_3)(_4))
      // Note: We cannot add a fallthrough case because it would make ambigious
      // methods The "best match" is the bottom match, but there is no "better"
      // match that works for the number of values we can decompose into. Thus,
      // we cannot easily make this error out cleanly.
...
template <class T, class F> constexpr auto do_on_bindings(T &t, F &&f) {
    ...
    detail::do_on_bindings(
        ...
        [&val, &begin, end, &options_map = std::as_const(options_map),
         &positionals_decoded](auto &&members) {
          ...
          // 2. we encountered something we don't know how to parse and we have
          // passed all of our positional args
          if (old_begin == begin) {
            ...
            for (auto const &res : results) {
              if (auto const *usage = std::get_if<UsageError>(&res)) {
                ...
                // assert that we don't have a flag error. We should never have
                // two flags fail
                assert(flag_error == nullptr);
                flag_error = fl;
              } else if (auto const *p =
                             std::get_if<detail::PositionalParseError>(&res)) {
                // assert that we don't have a positional error. We should never
                // have two pos flags fail
